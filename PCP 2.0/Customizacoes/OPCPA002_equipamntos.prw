...
Static Function MenuDef()
	...
	For nx := 1 to Len(_aGrpUser)
	  _IdGrpUser := UsrRetGrp(RetCodUsr())[nx]
		If _IdGrpUser $ "000007/000000"
			ADD OPTION _aRotina Title "PFI Equipamentos"	ACTION "U_OPEQUIPO(4)"			OPERATION MODEL_OPERATION_VIEW		ACCESS 0 // Por Samuel mirnada 04112021
	...
	ADD OPTION _aRotina Title "Selecionar OP"		ACTION "U_SETMK()"						OPERATION MODEL_OPERATION_VIEW		ACCESS 0
...
User Function PP002A01(n_Opcao,c_OP,c_ProdDe,c_ProdAta,c_Unidade,nSelect)
	...
	Private _nSelectOP := nSelect
	Private nOP := c_OP
	Private nProdDe :=c_ProdDe
	Private nProdAte :=c_ProdAta
	Private nUidade := c_Unidade
...
Static Function _fTransfer()
	...
	For _nCount := 1 To Len(_aDadosSZS)
		...
		If _aDadosSZS[_nCount][N1_PROCESS] $ "1|2"
			...
			//±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±//
			//±± Validação para correção da data de validade na transferencia  |    Samuel Miranda  | Data |15/01/25 |±±//
			//±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±//          
			
			//Pega a Data de Validade de Destino
			If SB8->(DbSeek(xFilial("SB8")+_aDadosSZS[_nCount][N1_PRODUTO]+_aDadosSZS[_nCount][N1_LOCDES]+_aDadosSZS[_nCount][N1_LOTECTL]))
				//Verifica se a Data de validade é menor que da Tadabase
				iF SB8->B8_DTVALID < dDataBase
					//Atribui a tada de validade conforme informado na SB1->B1_PRVALID 
					_dDtVldDes := dDataBase + SB1->B1_PRVALID  //SB8->B8_DTVALID
				  Else
					//Se não estiver vencido pega a data da SB8->B8_DTVALID
					_dDtVldDes := SB8->B8_DTVALID
				EndIf
			Else
				//Se a data de origem for menos que a database 
				If _dDtVldOri < dDataBase
					//Atribui a tada de validade conforme informado na SB1->B1_PRVALID 
					_dDtVldDes := dDataBase + SB1->B1_PRVALID  //SB8->B8_DTVALID
				Else
					//A data de destino recebe a data de Origem
					_dDtVldDes := _dDtVldOri
				EndIf
			EndIf
		...
		If _aDadosSZS[_nCount][N1_PROCESS] $ "1|2"
			...
			Else
				...
				If _aDadosSZS[_nCount][N1_QUANT] < _aDadosSZS[_nCount][N1_QTDSOLI]
					...
					// Executa a rotina MATA380
					MSExecAuto({|x,y| Mata380(x,y)},_aMata380,3)
					//MSExecAuto({|x,y,z| mata380(x,y,z)},aVetor,3,aEmpenhos)
