...
/*/{Protheus.doc} OPCPA002
	...
	/*/
User Function OPCPA002(c_Filtra)
...
Static Function MenuDef()
	...
	// Configiração para impressão de OP conforme solicitação Chamado numero  12739 Ecir |Por Samuel Miranda 24/06/2021
	Private _aGrpUser  := UsrRetGrp(RetCodUsr()) //Pega grupos do usuário
	//Private _IdGrpUser := UsrRetGrp(RetCodUsr())[1]

	ADD OPTION _aRotina Title "Visualizar"	 		ACTION "VIEWDEF.OPCPA002"				OPERATION MODEL_OPERATION_VIEW		ACCESS 0
	...
	// Verifica se o Usuário logado  pertence ao grupos de acesso |Por Samuel Miranda 24/06/2021
	For nx := 1 to Len(_aGrpUser)
	  _IdGrpUser := UsrRetGrp(RetCodUsr())[nx]
		...
		EndIf
	Next nx
	ADD OPTION _aRotina Title "PFI Ortopedia"	ACTION "U_xMTR797X(5)"						OPERATION MODEL_OPERATION_VIEW		ACCESS 0
	ADD OPTION _aRotina Title "Etiq. Separação"	ACTION "U_RESTR036"					    	OPERATION MODEL_OPERATION_VIEW		ACCESS 0
	//ADD OPTION _aRotina Title "Relatório de Log's"	ACTION "U_PP002A01(4)"				OPERATION MODEL_OPERATION_VIEW		ACCESS 0 RESTR036
...
Static Function ModelDef()
	...
	_oModel := MPFormModel():New("U_OPCPA002",,,_bCommit) 
...
Static Function _fExecProd(n_Opcao)
	...
	//MsgAlert("Não existe dados para os parâmetros informados!" + CRLF + CRLF + "Verifique.1")
	//MsgAlert("Não existe dados para os parâmetros informados!" + CRLF + CRLF + "Verifique.2")
	//MsgAlert("Não existe dados para os parâmetros informados!" + CRLF + CRLF + "Verifique.3")
...
Static Function _fMakeQry(n_Opcao)
	...
	If n_Opcao == 1
		...
		MemoWrite("LOGS_PROGRAMAS\OPCPA002\QUERY_ALTER.sql",GetLastQuery()[2])
	...
	ElseIf n_Opcao == 2
		BeginSql Alias _cAlias
			...
			INNER JOIN (
				...
				WHERE
					SD4.D4_FILIAL = %xFilial:SD4%
					AND SD4.D4_OP = %Exp:_cNumOpDe%  // Alterado de : >= para : =
					//AND SD4.D4_OP <= %Exp:_cNumOpAte% // Alterado pra Fazer uma OP por vez por: SAMUEL 23122020
					AND SD4.D4_COD >= %Exp:_cProdDe%
					AND SD4.D4_COD <= %Exp:_cProdAte%
					AND SD4.%NotDel%
		...
		MemoWrite("LOGS_PROGRAMAS\OPCPA002\QUERY_alt.sql",GetLastQuery()[2])
	...
	ElseIf n_Opcao == 3
		...
		MemoWrite("LOGS_PROGRAMAS\OPCPA002\QUERY_3.sql",GetLastQuery()[2])

		// Enquanto não for final de arquivo
		While (_cAlias)->(!Eof())
			SD4->(DbSetOrder(2))	// D4_FILIAL+D4_OP+D4_COD+D4_LOCAL
			If SD4->(DbSeek(xFilial("SD4")+PadR((_cAlias)->ZS_OP,TamSX3("D4_OP")[1])+PadR((_cAlias)->ZS_PRODUTO, TamSX3("D4_COD")[1])+(_cAlias)->ZS_LOCDES))
				// Enquanto não for final de arquivo e for o mesmo registro
				While SD4->(!Eof()) .And. SD4->(D4_FILIAL+D4_OP+D4_COD+D4_LOCAL) == xFilial("SD4")+PadR((_cAlias)->ZS_OP,TamSX3("D4_OP")[1])+PadR((_cAlias)->ZS_PRODUTO, TamSX3("D4_COD")[1])+(_cAlias)->ZS_LOCDES
...
Static Function _fProcessa(l_Tranfere)
	...
	If _lContinua
		 //Imprimi a etiqueta somente para ortopedia
		 If _cArmEmpe == 2
			//cMensagem := "Deseja imprimir a Etiqueta"
			cMensagem := '<b>Empenho realizado com sucesso! !</b>'
			cMensagem += '<br>'
			cMensagem += '<br>'
			cMensagem += '<b>Deseja imprimir a Etiqueta! </b>'
			If MsgYesNo(cMensagem, "A T E N Ç Ã O !")
				RptStatus({|lEnd| U_RESTR36Imp(@lEnd,_cNumOpDe,.T.)},"Imprimindo, aguarde...")
			EndIf
		 Else
...
Static Function _fExcEmpe()
	...
	Local _cPath := "LOGS_PROGRAMAS\OPCPA002\MsExecAuto\MATA380\"
	Local _cNome := "LOG_EXCLUIR_MATA380_U_" + cNamUser + "_D_" + DToS(dDataBase) + "_H_" + Strtran(SubStr(Time(),1,5),":","") + ".log"
...
Static Function _fTransfer()
	...
	Local _cNome := "MATA261_U_" + cNamUser + "_D_" + DToS(dDataBase) + "_H_" + Strtran(SubStr(Time(),1,5),":","")
...
Static Function _fEmpenho(l_Empenhar)
	...
	Local _cPath := "LOGS_PROGRAMAS\OPCPA002\MsExecAuto\MATA380\"
	Local _cNome := "LOG_EMPENHAR_MATA380_U_" + cNamUser + "_D_" + DToS(dDataBase) + "_H_" + Strtran(SubStr(Time(),1,5),":","") + ".log"
			...
			Else
				...
				If _aDadosSZS[_nCount][N1_QUANT] < _aDadosSZS[_nCount][N1_QTDSOLI]
					...
					If !Empty(_aDadosSZS[_nCount][N1_TRTSD4]) .And. !Empty(_aDadosSZS[_nCount][N1_LOTECTL])
						...
						//Aadd(_aMata380,{"D4_TRT",_aDadosSZS[_nCount][N1_TRTSD4],Nil})
					Else
					//Força a inclusão do número da sequencia // Samuel Miranda 08122021
						If _aDadosSZS[_nCount][18]=='S'
							Aadd(_aMata380,{"D4_TRT",_aDadosSZS[_nCount][4],Nil})
						End
					EndIf
...
Static Function _fGravaSZS()
	...
	For _nCount := 1 To Len(_aDadosSZS)
		...
		// Necessário forçar a gravação do ZS_PRODUTO porque o model não estava gravando
		RecLock("SZS", .F.)
			SZS->ZS_PRODUTO := _aDadosSZS[_nCount][N1_PRODUTO]
		MsUnlock()
	Next
...
Static Function _fExecDevo()
	...
	Else
		...
		If _lContinua
			//Retirado com Solicitação Chamado #19794 - Sr. Ecir PCP/Equipamentos
...
Static Function _fExcAllEm()
	...
	Local _cPath := "LOGS_PROGRAMAS\OPCPA002\MsExecAuto\MATA380\"
	Local _cNome := "LOG_EXCLUIR_ALL_MATA380_U_" + cNamUser + "_D_" + DToS(dDataBase) + "_H_" + Strtran(SubStr(Time(),1,5),":","") + ".log"
...
Static Function _fExcluOP(_cNumOP)
	...
	SC2->(DbSetOrder(1))//FILIAL + NUM + ITEM + SEQUEN + ITEMGRD
 	SC2->(DbSeek(xFilial("SC2")+_cNumSC2+_cItem+_cSequen))
...
//±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
//±± Função    |  _nSeqOp   | Autor: Samuel Miranda     | Data   08/12/21   º±±
//±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
