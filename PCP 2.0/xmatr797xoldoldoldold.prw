...
Local nBegin
Local _nAux, _nAux1, _nAux2, _nXXX
Local nQtdInsp	:= 0
Local nI		:= 0
Local cQueryOP := ""
Local _nxRec := 0
Local _cInstru, cDescri
...
If Len(Alltrim(_cInstru)) > 45
	_nAux1 := Int(Len(Alltrim(_cInstru))/45)
EndIf
If _nAux1 <> 0
	_nAux1 := (_nAux1+1) * 10
Else
...
cDescri := IIF(lSH8,SH8->H8_RECURSO,SG2->G2_RECURSO)+" "+SUBS(SH1->H1_DESCRI,1,25)
If Len(Alltrim(cDescri)) > 45
	_nAux2 := Int(Len(Alltrim(cDescri))/45)
EndIf
If _nAux2 <> 0
	_nAux2 := (_nAux2+1) * 10
Else
...
_nAux := IIf(_nAux1 > _nAux2, _nAux1, _nAux2)
_nXXX := 0
...
For nBegin := 1 To Len(cDescri) Step 45
	oPrinter:Say(li+005,022,Substr(cDescri,nBegin,45),oFontT)
	li+=10
	If li> nMaxLin-59
		li:= 0
		oPrinter:EndPage()
