...
Private nMaxLin  := 0
Private nMaxCol  := 0
//Private lItemNeg := GetMv("MV_NEGES!TR") .And. MV_PAR11 == 1
...
Else
		...
		For _nCount := 2 TO Len(aArray)
			...
			iF !Empty(_cAuxEnd) 
			  ...
			  Else
				...
				oPrinter:Say(Li+007,288,cQtd,oFontTB) //"QUANTIDADE"       272
				oPrinter:Say(Li+007,342,aArray[_nCount][4],oFontTB) //"UM"
				...
				oPrinter:Say(Li+007,550,AllTrim(cQtd2),oFontTB)      //"2 UM"
				Li+=10
		...
		If dbSeek(xFilial("SZ2")+cProduto)	
			...
			For _nCount := 1 to Len(_aBmp) // Le o Array com as Figuras a serem impressas
				...
				If File(_cBmp)
					...
					//Adicionado para imprim duas paginas em branco, Solicitado Por: Luis Riardo (PCP)
					If File(_cBmp)
						For _xpage := 1 to 2
							oPrinter:SetLandscape()
							oPrinter:StartPage()
	...
	If _lImpEsp .And. !Empty(_aProdSeq1)
		...
		CabecOp(nPagina,oPrinter,0,.F.,_aProdSeq1,.T.)		
	EndIf		
EndIf
...
If li == 20
	oPrinter:FWMSBAR("CODE128" /*cTypeBar*/,2.1/*nRow*/,0.3/*nCol*/,AllTrim(cCode)/*cCode*/,oPrinter/*oPrint*/,/*lCheck*/,/*Color*/,/*lHorz*/,/*nWidth*/,0.8/*nHeigth*/,/*lBanner*/,/*cFont*/,/*cMode*/,.F./*lPrint*/,/*nPFWidth*/,/*nPFHeigth*/,/*lCmtr2Pix*/) 
Else
...
oPrinter:SayAlign(Li+30	,480,"F1 09-01 Rev.07",oFontTB,nMaxCol-10,200,,0) // Leonardo Vieira 27/04/2022

oPrinter:SayAlign(Li+05	,125,cCabec1,oFont14N,600,200,,0,,,.T.)
...
oPrinter:Say(Li+09,212,SubStr(IIf(Empty(a_ProdSeq1),aArray[1][2],a_ProdSeq1[3]),01,37),oFont14N,600)
oPrinter:Say(Li+18,212,SubStr(IIf(Empty(a_ProdSeq1),aArray[1][2],a_ProdSeq1[3]),38,70),oFont14N,600)
...
If !(Empty((_cAliasTop)->C2_OBS))
	oPrinter:Say(li-3,5,STR0033)//"Observacao: "
	For nBegin := 1 To Len(Alltrim((_cAliasTop)->C2_OBS)) Step 65
		oPrinter:Say(li-3,60,Substr((_cAliasTop)->C2_OBS,nBegin,65))
...
If l_Tudo
	...
	oPrinter:Say(Li+008,002,STR0034,oFontTB) //"CODIGO"
	oPrinter:Say(Li+008,072,STR0035,oFontTB) //"DESCRICAO"
	oPrinter:Say(Li+008,302,"QTDE",oFontTB)  //"QUANTIDADE"    272
	oPrinter:Say(Li+008,342,STR0037,oFontTB) //"UM"
	oPrinter:Say(Li+008,372,STR0038,oFontTB) //"ARM"
	oPrinter:Say(Li+008,392,STR0039,oFontTB) //"ENDERECO"
	oPrinter:Say(Li+008,462,"2a UM" ,oFontTB)//"2a UM Sigla"
	oPrinter:Say(Li+008,492,"LOTE" ,oFontTB) //"LOTE"
	oPrinter:Say(Li+008,552,"Qt 2a UM",oFontTB) //"2a UM QT"
	...
	While !Eof() .And. Eval(&cSeekWhile)
		...
		If lSH8
			While !Eof() .And. SH8->H8_FILIAL+SH8->H8_OP+SH8->H8_OPER == xFilial()+(_cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)+SG2->G2_OPERAC
				...
				If mv_par13 == 2
						If !Empty(SG2->G2_DTINI) //Data de validade inicial
							If SG2->G2_DTINI > (cAliasTop)->C2_DATPRI //Data prevista de inicio
								SH8->(dbSkip())
								Loop
							EndIf
						EndIf
						If !Empty(SG2->G2_DTFIM)//Data de validade final
							If SG2->G2_DTFIM < (cAliasTop)->C2_DATPRI //Data prevista de inicio
								SH8->(dbSkip())
								Loop
							EndIf
						EndIf
				...
				If !_lFirst1
					...
					oPrinter:Say(li+008,002,"OPER",oFontTB)				//"OPERACAO"
					oPrinter:Say(li+008,022,STR0047,oFontTB)			//"RECURSO"
					oPrinter:Say(li+008,172,STR0048,oFontTB)			//"FERRAMENTA"
					oPrinter:Say(li+008,272,"TEMPO",oFontTB)			//"TEMPO"
					oPrinter:Say(li+008,322,"INSTRUÇÕES",oFontTB)		//"Instrucoes"
					oPrinter:Say(li+008,522,"Proc.Apli.DQL",oFontTB)	//"DQL"
					li+=10
				EndIf
				_lFirst1 := .T.
...
Local cCabec1 := " ROTEIRO DE OPERACOES "+(_cAliasTop)->C2_ROTEIRO+"  O.P.: "+(_cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)	
...
//li+=10
oPrinter:SayAlign(Li,150,cCabec1,oFont14N,nMaxCol-10,,,0)
li+=20
...
oPrinter:Say(li+007,002,SG2->G2_OPERAC,oFontT) // Operacao 
oPrinter:Say(li+007,022,IIF(lSH8,SH8->H8_RECURSO,SG2->G2_RECURSO)+" "+SUBS(SH1->H1_DESCRI,1,25),oFontT)  // Recurso 

If Empty(SG2->G2_FERRAM)
	oPrinter:Say(li+007,172,SG2->G2_FERRAM+" "+SUBS(SH4->H4_DESCRI,1,20),oFontT) //  Ferramenta
 Else
	nLiOld := li
	_cAlias := Alias()
	dbSelectArea("SH3")
	...
	dbSeek(SG2->(G2_FILIAL+G2_PRODUTO+G2_CODIGO+G2_OPERAC))
	While !Eof() .And. SG2->(G2_FILIAL+G2_PRODUTO+G2_CODIGO+G2_OPERAC) == SH3->(H3_FILIAL+H3_PRODUTO+H3_CODIGO+H3_OPERAC)
		oPrinter:Say(li+007,172,SH3->H3_FERRAM+" "+SUBS(Posicione("SH4",1,xFilial("SH4")+SH3->H3_FERRAM,"H4_DESCRI"),1,20)) // Ferramenta
		li+=10
		dbSkip()
	EndDo
	li := nLiOld
...
oPrinter:Say(li+007,272,Str(SG2->G2_TEMPAD,5,2),oFontT) // Tempo 
...
Next nBegin
Li := Li - (10*_nVezes)
oPrinter:Say(li+007,522,SH1->H1_XDQL,oFontT) // DQL 
//Li := Li + (5*_nVezes)
...
//Inicio da Impressão do recurso alternativo
...
If !empty(SG2->G2_RECURSO)
	...
	If _nxRec > 0
		oPrinter:Box(Li-5,001,Li+10,nMaxCol-200) // Box do recurso alternativo
...
If _nRecurso == "BG01" // $ "12010|BG01" 
	...
	li += 200
EndIf
//Impressão do codigo de barras //Por Samuel Miranda 14/07/2021
If _nRecurso $ "12025|12029|12023 "
	li += 5 //20
	_nPosLinha := li / 11.6
...
_aArea := GetArea()
_cQuery := ""
_cQuery += " SELECT QP8_ENSAIO ENSAIO,"+CRLF
_cQuery += " QP1_DESCPO DESCRICAO,"+CRLF
_cQuery += " QP8_LABOR LABOR,"+CRLF
_cQuery += " QP8_METODO METODO,"+CRLF
_cQuery += " QP8_TEXTO TEXTO,"+CRLF
_cQuery += " ' ' MINIMO,"+CRLF
_cQuery += " ' ' MAXIMO , "+CRLF
_cQuery += " QP8_SEQLAB ORDEM,"+CRLF
_cQuery += " QP8.R_E_C_N_O_ REG "+CRLF
...
_cQuery += " AND QP8_CODREC='"+(_cAliasTop)->C2_ROTEIRO+"' " //Adicionado Por Samuel Miranda 25/04/2024 (Inluir o codigo do ROTEIRO na QP8)
...
//_cQuery += " AND QP7_XROTRO = '"+(_cAliasTop)->C2_ROTEIRO+"'"+CRLF //Campo customizado, para vincular a inpeção com o Roteiro.
...
_cQuery += " WHERE "+CRLF
_cQuery += " QQK_FILIAL = '"+xFilial("QQK")+"' "+CRLF
_cQuery += " AND QQK_CODIGO = '"+(_cAliasTop)->C2_ROTEIRO+"' "+CRLF
_cQuery += " AND QQK_PRODUT = '"+(_cAliasTop)->C2_PRODUTO+"' "+CRLF
...
MemoWrit("XMATR820.SQL",_cQuery) //"+(_cAliasTop)->C2_PRODUTO+"
//MemoWrit("XMATR820"+(_cAliasTop)->C2_PRODUTO+".SQL",_cQuery) //
...
//Pirolo - Melhoria solicitada em 23/05/2019 - Imprimir  o quadro de inspeções x vezes conforme amarração na tabela QA6
...
For nI := 1 to nQtdInsp
	...
	While QRY->(!Eof())
		...
		If li > nMaxLin-59
			...
			oPrinter:Say(li+007,002,"ENSAIO" 				 , oFontTB)
			oPrinter:Say(li+007,037,"DESCRICAO" 			 , oFontTB)
			oPrinter:Say(li+007,187,"LABOR" 				 , oFontTB)
			oPrinter:Say(li+007,350,"ESPECIFICAÇÕES/MEDIÇÕES", oFontTB)
			oPrinter:Say(li+007,551,"RESULTADO" 			 , oFontTB)
			Li+=10
		EndIf
		...
		QRY->(dbSkip())
	EndDo
	//oPrinter:Say(li,005,cValtochar(li) 	  , oFontTB)//01
	...
	If _lTemInsp
		...
		//VerPag2(oPrinter)
		Li+=20
		oPrinter:Box(Li,001,Li+15,200) // 
		oPrinter:Say(li+010,005,"Verificado por:" 	  , oFontTB)//01
		
		oPrinter:Box(Li,201,Li+15,400) // 
		oPrinter:Say(li+010,205,"Cód.do instrumento:" , oFontTB)//01
		
		oPrinter:Box(Li,401,Li+15,nMaxCol-10) // 
		oPrinter:Say(li+010,405,"Data da verificação:", oFontTB)//01
		
		li+=15
		oPrinter:Box(Li,001,Li+15,200) // 
		oPrinter:Say(li+010,005,"Verificado por:" 	  , oFontTB)//02
		
		oPrinter:Box(Li,201,Li+15,400) //
		oPrinter:Say(li+010,205,"Cód.do instrumento:" , oFontTB)//02
		
		oPrinter:Box(Li,401,Li+15,nMaxCol-10) //
		oPrinter:Say(li+010,405,"Data da verificação:", oFontTB)//02
		
		li+=15
		oPrinter:Box(Li,001,Li+15,200) // 
		oPrinter:Say(li+010,005,"Verificado por:" 	  , oFontTB)//03
		
		oPrinter:Box(Li,201,Li+15,400) // 
		oPrinter:Say(li+010,205,"Cód.do instrumento:" , oFontTB)//03
		
		oPrinter:Box(Li,401,Li+15,nMaxCol-10) // 
		oPrinter:Say(li+010,405,"Data da verificação:", oFontTB)//03
		
		li+=15
		oPrinter:Box(Li,001,Li+15,200) // 
		oPrinter:Say(li+010,005,"Verificado por:" 	  , oFontTB)//04
		
		oPrinter:Box(Li,201,Li+15,400) // 
		oPrinter:Say(li+010,205,"Cód.do instrumento:" , oFontTB)//04
		
		oPrinter:Box(Li,401,Li+15,nMaxCol-10) // 
		oPrinter:Say(li+010,405,"Data da verificação:", oFontTB)//04
		
		li+=15
		oPrinter:Box(Li,001,Li+15,300) // 
		oPrinter:Say(li+010,005,"Quantidade aprovada:" 	  , oFontTB)
		
		oPrinter:Box(Li,301,Li+15,nMaxCol-10) // 
		oPrinter:Say(li+010,305,"Quantidade Reprovada:" , oFontTB)
...
Next nI
oPrinter:Say(Li,nMaxCol-10,CValToChar(Li),oFontTB) //imprime nuemro da linha	
dbCloseArea()
RestArea(_aArea)
...
Static Function VerPag2(oPrinter)

IF li + 50 > (nMaxLin)						// Li > 55
	li := 15
	oPrinter:EndPage()
	nPagina++
	oPrinter:StartPage()
...
oPrinter:SayAlign(Li,001,CValToChar(Li)+" Cabec",oFont14N,nMaxCol-10,200,,0)
li+=20
