...
oPrinter:Box(Li,001,Li+25,250) //eRA Li+20
oPrinter:Box(Li,250,Li+25,500)
oPrinter:Box(Li,500,Li+25,nMaxCol-10)
...
Local cCabec1 :=""
Local cProdTo := Posicione("SB1",1,xFilial("SB1")+(_cAliasTop)->C2_PRODUTO, "B1_TIPO") 

If cProdTo == "PA"
	 cCabec1 := "ROTEIRO "+(_cAliasTop)->C2_ROTEIRO+ " DE OPERACOES NRO :"+(_cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD) 	//" ROTEIRO DE OPERACOES NRO :"
Else
	 cCabec1 := "ROTEIRO DE OPERACOES NRO :"+(_cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD) 	//" ROTEIRO DE OPERACOES NRO :"
EndIf
...
//   SG2->G2_LOTEPAD	 (_cAliasTop)->(C2_QUANT)
//   ---------------- X  --------------------
//   SG2->G2_TEMPAD				X

				//Peças				//Tempo
//nTempOper := ((SG2->G2_LOTEPAD)/(SG2->G2_TEMPAD))//*(_cAliasTop)->(C2_QUANT) // Tempo para fabricação de toda a OP na Operação atual

//oPrinter:Say(li+005,272,FmtHr(nTempOper),oFontT) //oPrinter:Say(li+005,272,FmtHr(SG2->G2_TEMPAD),oFontT)
//oPrinter:Say(li+005,272,FmtHr(SG2->G2_TEMPAD),oFontT)
oPrinter:Say(li+005,272,cvaltochar(SG2->G2_TEMPAD),oFontT) //oPrinter:Say(li+005,272,cvaltochar(Transform(SG2->G2_TEMPAD,"@R 99:99:99"),oFontT)
...
//Verificar que tem registro na primeira Query, pois precisar tra na primeira consulta.
//If nRec > 0
	// Incluido Claudio 07/10/2013 INICIO
	_aArea := GetArea()
	_cQuery := ""
	_cQuery += " SELECT "+CRLF
	_cQuery += "QP8_ENSAIO ENSAIO,"+CRLF
	...
	//_cQuery += " AND QP8_REVI='"+SG2->G2_REVIPRD+"'  " //Adicionado Por Samuel Miranda 25/04/2024 (Inluir o codigo DA REVISÃO na QP8)
	...
	//_cQuery += "     AND QP6_CODREC = '"+(_cAliasTop)->C2_ROTEIRO+"' "  ////Pirolo - Removido o comentario desta linha para obedecer o roteiro definido na OP
	...
	_cQuery += " WHERE QQK_FILIAL = '"+xFilial("QQK")+"' " +CRLF
	...
	_cQuery += " AND QQK_FERRAM = '"+SG2->G2_FERRAM+"' "+CRLF //_cQuery += " AND QQK_CHAVE = '"+SG2->G2_CHAVE+"' "+CRLF //Por Samuel
	_cQuery += " AND QQK_REVIPR = QP8_REVI "+CRLF
	_cQuery += " AND QQK.D_E_L_E_T_ = ' ' "+CRLF
	//faz um unio com as duas consultas
	_cQuery += " UNION ALL "+CRLF
	_cQuery += " SELECT DISTINCT QP7_ENSAIO ENSAIO,"+CRLF
	_cQuery += " QP1_DESCPO DESCRICAO,"+CRLF
	_cQuery += " QP7_LABOR LABOR,"+CRLF
	_cQuery += " QP7_METODO METODO,"+CRLF
	_cQuery += " ' ' TEXTO,"+CRLF
	_cQuery += " QP7_LIC MINIMO,"+CRLF
	_cQuery += " QP7_LSC MAXIMO,"+CRLF
	_cQuery += " QP7_SEQLAB ORDEM,"+CRLF
	_cQuery += " QP7.R_E_C_N_O_ REG "+CRLF
	_cQuery += " FROM "+RetSqlName("QQK")+" QQK "+CRLF
	...
	_cQuery += " AND QP7_REVI = '"+SG2->G2_REVIPRD+"'"+CRLF
	...
	_cQuery += "     AND QP6_PRODUT = '"+Alltrim((_cAliasTop)->C2_PRODUTO)+"' "+CRLF
	_cQuery += "     AND QP6_SITREV <> '1' "+CRLF    //Por Samuel Miranda colocado para não imprimir revisao bloqueadas 20190527
	//_cQuery += "     AND QP6_CODREC = '"+(_cAliasTop)->C2_ROTEIRO+"' " //Pega o Roteiro de produção da Ordem de produção //Pirolo - Removido o comentario desta linha para obedecer o roteiro definido na OP
	_cQuery += "     AND QP6.D_E_L_E_T_ = ' ') "+CRLF
	...
	_cQuery += " AND QQK_REVIPR = QP7_REVI "+CRLF //_cQuery += " AND QQK_CHAVE = '"+SG2->G2_CHAVE+"' "+CRLF //Por Samuel
	_cQuery += " AND QQK.D_E_L_E_T_ = ' ' "+CRLF
	_cQUery += " ORDER BY ORDEM, REG "+CRLF

	MemoWrit("XMATR820.SQL",_cQuery)//Nesse ponto ele salva indentado.
	...
	For nI := 1 to nQtdInsp
		...
		While QRY->(!Eof())
			...
			nEnsaio += 1  //AllTrim(QRY->ORDEM)
			If AllTrim(QRY->ORDEM) =  Padl(nEnsaio,2,"0") //PadL(cNFiscal, nTamanho, '0')
				//Adcionado por Samuel Miranda -25/04/2024 para imprimir somente uma vez e quando houver inspeção
				oPrinter:Box(Li,001,Li+20,035) // Ensaio - 035 Pos
				oPrinter:Box(Li,035,Li+20,185) // Descricao  - 150 Pos
				oPrinter:Box(Li,185,Li+20,220) // Laboratrio - 035 Pos
				oPrinter:Box(Li,220,Li+20,550) // Especificacao - 330 Pos
...
------------------------------------------------------------------------------------------------
Parâmetros:
	nTempo: Tempo (G2_TEMPAD)
------------------------------------------------------------------------------------------------
Retorno:
	Tempo formatado
**************************************************************************************************************
/*/
Static Function FmtHr(nTempo)

	Local nTempoMin := nTempo * 60 // nTempo * 60
	Local nHoras    := NoRound(nTempo, 0)
	Local nMinutos  := NoRound(nTempoMin - 60 * nHoras, 0)
	Local nSegundos := NoRound((nTempoMin - nMinutos - 60 * nHoras) * 60, 0)
