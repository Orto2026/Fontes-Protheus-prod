...
Local cTitulo := " ORDEM DE DISTRIBUIÇÃO  "
Local cTitulo2:= "O.D. :"+(_cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD) + "LOTE: " + (_cAliasTop)->C2_LOTECTL + If((_cAliasTop)->C2_LOTECTL <> '',"" ,"")  //samuel
...
oPrinter:Box(Li,001,li+20,nMaxCol-10)
oPrinter:Say(Li+10,050,"Rastrear  - Procedimento Aplicável POP"+AllTrim(GetMv("MV_XPROCMP")),oFont14N)
Li += 20
		...
		If lSH8
			While !Eof() .And. SH8->H8_FILIAL+SH8->H8_OP+SH8->H8_OPER == xFilial()+(_cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)+SG2->G2_OPERAC
				If !_lFirst1
					...
					oPrinter:Say(li+08,522,"Proc.Apli.POP",oFontTB)	//"DQL"
					li+=10
				EndIf
				_lFirst1 := .T.
