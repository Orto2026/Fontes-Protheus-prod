...
//	If AllTrim(SB1->B1_LOCPAD) $ "92" 
//		DbSelectArea("SD4")
//		SD4->(dbSetOrder(2)) //2 	D4_FILIAL + D4_OP + D4_COD + D4_LOCAL 	Ord Producao + Produto + Armazem
//		If SD4->(DbSeek(xFilial("SD4")+cOp+D4_COD+D4_LOCAL))
//		//If dbSeek(SD4->(D4_FILIAL+D4_COD+D4_LOCAL))
//			RecLock("SD4", .T.)
//				D4_QTDEORI	:= Round(SD4->D4_QUANT,2)
//			SD4->(MsUnlock())
//		EndIf
//	EndiF
//	DbSkip()
//EndDo
	...
	While !EoF() .AND. SD4->D4_OP == cOp
		...
		//If AllTrim(SB1->B1_LOCPAD) $ "92/10" 
		//	DbSelectArea("SD4")
		//	SD4->(dbSetOrder(2)) //2 	D4_FILIAL + D4_OP + D4_COD + D4_LOCAL 	Ord Producao + Produto + Armazem
		//	If SD4->(DbSeek(xFilial("SD4")+cOp+D4_COD+D4_LOCAL))
		//	//If dbSeek(SD4->(D4_FILIAL+D4_COD+D4_LOCAL))
	...
	//Seleciona o Alias SD4
	DbSelectArea("SD4")
	DbsetOrder(2)
	//SD4->(DbGoTo())
	DbSeek(xFilial("SD4")+cOp)
	//Seleciona o Alias SB1
	DbSelectArea("SB1")
	DbSetOrder(1)
	DbSeek(xFilial("SB1")+SD4->D4_COD)
	//Entra no Laço
	WHILE !SD4->(EOF()) .AND. cOp==SD4->D4_OP
		cDescri   := POSICIONE("SB1",1,xFilial("SB1")+SD4->D4_COD,"B1_DESC")
		_cUmProd  := POSICIONE("SB1",1,xFilial("SB1")+SD4->D4_COD,"B1_UM")
			reclock("SD4",.F.)
				//SD4->D4_XDESC := cDescri
				SD4->D4_XUM     := _cUmProd //Grava a Unida de medida do produto
			SD4->(MSUNLOCK())
		DBSKIP()
	ENDDO
