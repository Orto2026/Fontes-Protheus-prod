...
//--> Variaveis para Importacao - Serao Alimentadas no aDi
Private _aDescAux	:= {} // incluido por Mauricio Prado - 19/11/13
Private _cDescPr	:= "" // incluido por Mauricio Prado - 19/11/13
Private _cCodGtin   := "" // incluido por Samuel Miranda - 30/09/2025
_lFirstEX 	:= .T.		  // Variavel Para Controle de Exportacao
_lIsImp  	:= .F.
_nDi 		:= ""
_dDi 		:= ""
_cLD 		:= ""
...
_dLD     	:= ""
_cForn   	:= ""
_cAcdraw   	:= "" 	  // Claudio 16/03/2015
_cVtrans  	:= ""     // Claudio 16/03/2015
_cInterm  	:= ""     // Claudio 17/03/2015
If FunName() == "SPEDNFSE"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_DTVALID,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
				...
				//³Bloco Especifico para ORTOSINTESE     - Dados Adicionais (Mauricio 19/11/13) ³
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				// inicio - dados adicionais pedido - Mauricio Prado - 02/01/14	
				aAreaSA1 := SA1->(GetArea())
				SA1->(dbSetOrder(1))    
				
				If !Empty(SC5->C5_XCOND) .AND. !("OBS.COND.PAGAMENTO:" $ cMensCli)
					cMensCli += "OBS.COND.PAGAMENTO: "+Alltrim(SC5->C5_XCOND)+" / "
				Endif 
				
				If !Empty(SC5->C5_XENDENT) .AND. !("ENDEREÇO ENTREGA:" $ cMensCli)
					cMensCli += "ENDEREÇO ENTREGA: "+Alltrim(SC5->C5_XENDENT)+" / "
				Endif
				
				/* MAURICIO - RETIRADO A PEDIDO DE VAGNER E FABIANA(EQUIPAMENTOS)
				If !Empty(SC5->C5_XOBSER) .AND. !("OBS.PEDIDO:" $ cMensCli)
					cMensCli += "OBS.PEDIDO: "+Alltrim(SC5->C5_XOBSER)+" / "
				Endif
				*/
				If !Empty(SC5->C5_XOBSCRI) .AND. !("OBS.CRITICA:" $ cMensCli)
					cMensCli += "OBS.CRITICA: "+Alltrim(SC5->C5_XOBSCRI)+" / "
				Endif 
				
			  //	If !('Pagamentos'$cMensCli) .And. Len(aDupl) > 12   // alterado de 9 para 10
				//	cMensCli += NfeCob1(aDupl)
			//	EndIf
	
				RestArea(aAreaSA1)
				// fim
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				//³Bloco Especifico para ORTOSINTESE     - Dados Adicionais (Mauricio 28/06/11) ³
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					...
					If SC5->( FieldPos("C5_XMENPAD") ) > 0 .And.  !Empty(SC5->C5_XMENPAD) .And. !AllTrim(FORMULA(SC5->C5_XMENPAD)) $ cMensFis
						If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
							cMensFis += " "
						EndIf
						cMensFis += AllTrim(FORMULA(SC5->C5_XMENPAD))
					EndIf
					...
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			            ...
			            EndIf
	       				...
	       				// inicio - tratamento descricao complementar do produto (ANVISA) - Mauricio Prado - 19/11/13
					If !(SF2->F2_TIPO$"D") .And. SF2->F2_EST <> "EX"
						dbSelectArea("SB1")
						dbSetOrder(1)
						...
						If !Empty(SB1->B1_XANVISA) 
							If SB1->B1_XANVEMP ="1"
								If SB1->B1_XANVISA $ ""
								//Atleração apedido da Lullis dos assuntos Regulatorios 01/08/2025 Samuel Miraanda (Codigos incluido 0130/9002) (removido /0051/0052/0053)
								_cDescPr := " REGISTRO ANVISA: 1022371"+SB1->B1_XANVISA  //+ " - " +"GTIN: "+IIF(!Empty(SB1->B1_CODGTIN),Alltrim(SB1->B1_CODGTIN),"SEM GTIN")	
								aAdd(_aDescAux, ;
								{Len(aProd)+1 , ; 	// 01 - Cod Item
								Alltrim(_cDescPr); 	// 02 - Descricao Prd
								})
								EndIf					
							ElseIf SB1->B1_XANVEMP ="2"
							//Ateração feita apedido da Lullis dos assuntos Regulatorios 11/01/20 Samuel Miraanda (Codigo 0001)
   							If SB1->B1_XANVISA $ "0003/0004/0005/0006/0007/0008/0009/0010/0011/0012/0013/0014/0015/0016/0017/0018/0019/0020/0021/0022/0023/0024/0025/0026/0027/0028/0029/0030/0031" 
   							//Atleração apedido da Lullis dos assuntos Regulatorios 01/08/2025 Samuel Miraanda (Codigos incluido 0018/0019/0020/0021/0022/0023/0024/0025/0026/0027/0028/0029/0030/0031) (removido 0002)
							_cDescPr := " REGISTRO ANVISA: 8120219"+SB1->B1_XANVISA	 //+ " - " +"GTIN: "+IIF(!Empty(SB1->B1_CODGTIN),Alltrim(SB1->B1_CODGTIN),"SEM GTIN")	
							//Adicionado por Samuel Miranda 29/09/2025
							//_cDescPr := "GTIN: "+Alltrim(Posicione("SB1",1,xFilial("SB1")+SB1->B1_COD,"B1_CODGTIN")) 				
   							aAdd(_aDescAux, ;
								{Len(aProd)+1 , ; 	// 01 - Cod Item
								Alltrim(_cDescPr); 	// 02 - Descricao Prd
								})
                        ...
                        // Equipamentos Codigo Anvisa e Registro
						If !Empty(SB1->B1_XCOD) 
							
							_cDescPr := " CODIGO ANVISA:"+SB1->B1_XCOD
							aAdd(_aDescAux, ;
							{Len(aProd)+1 , ; 	// 01 - Cod Item
							Alltrim(_cDescPr)	; 	// 02 - Descricao Prd
							})
							...
							If SB1->B1_XANVEMP ="1"
								...
								_cDescPr := " REGISTRO ANVISA: 1022371"+SB1->B1_XANVISA
								aAdd(_aDescAux, ;
								{Len(aProd)+1 , ; 	// 01 - Cod Item
								Alltrim(_cDescPr)	; 	// 02 - Descricao Prd
								})
							...
							ElseIf SB1->B1_XANVEMP ="2"
   							...
   							_cDescPr := " REGISTRO ANVISA: 8120219"+SB1->B1_XANVISA					
   							aAdd(_aDescAux, ;
								{Len(aProd)+1 , ; 	// 01 - Cod Item
								Alltrim(_cDescPr)	; 	// 02 - Descricao Prd
								})
						...
						//LOTE DO PRODUTO
						If !Empty((cAliasSD2)->D2_LOTECTL)
						//_cDescPr := "LOTE: "+(cAliasSD2)->D2_LOTECTL+" "+IIF(Alltrim(SB1->B1_XSTERI)="2",";VAL. INDETERMINADA",IIF(!Empty((cAliasSD2)->D2_DTVALID),";VAL.: "+SUBS((cAliasSD2)->D2_DTVALID,5,2)+"/"+SUBS((cAliasSD2)->D2_DTVALID,1,4)+"",""))
						...
						If !Empty(SC6->C6_PEDCLI)
							_cDescPr := "P.V.: "+SC6->C6_PEDCLI+" "
							aAdd(_aDescAux, ;
							{Len(aProd)+1 , ; 	// 01 - Cod Item
							Alltrim(_cDescPr)	; 	// 02 - Descricao Prd
							})
						EndIf
						If !Empty(SC6->C6_PEDFOR)
							_cDescPr := "P.C.: "+SC6->C6_PEDFOR+" "
							aAdd(_aDescAux, ;
							{Len(aProd)+1 , ; 	// 01 - Cod Item
							Alltrim(_cDescPr)	; 	// 02 - Descricao Prd
							})
					...
					Endif
						...
						// ESPECIFICO ORTOSINTESE
						If SB1->B1_TIPO ="PA"
							If !Empty(SB1->B1_XDMCP) 
							cDescProd := SB1->B1_XDMCP
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			Else
				...
				//³Bloco para Tratar Importacoes - Especifico para Cliente - CUSTOMIZACOES - DEMA³
				...
				If _lFirstEX 
					...
					While !Eof() .And. SF1->F1_FILIAL+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA == ;
						...
						If VALTYPE("D1_XDI") # "U" .And. !Empty(SD1->D1_XDI)
						   _lIsImp	 := .T.
							_nDi 	 := AllTrim(SD1->D1_XDI)
							_dDi 	 := SD1->D1_XDTDI
				   		    _cLD 	 := SD1->D1_XLOCDES	// -- Local do Desembarque
				     		_cUFLD   := SD1->D1_XESTDES	// -- UF do Desembarque 
				       	    _dLD     := SD1->D1_XDATDES	// -- DATA do Desembarque 
				        	_cForn   := SD1->D1_FORNECE
							_cAcdraw := SD1->D1_ACDRAW   // Claudio 16/03/2015
							_cVtrans := SD1->D1_VTRANS 	 // Claudio 16/03/2015
							_cInterm := SD1->D1_XINTERM  // Claudio 17/03/2015
							Exit
						EndIf
						dbSkip()
					EndDo
				...
				//³Fim do Bloco para Tratar Importacoes - Especifico para Cliente - CUSTOMIZACOES - DEMA³
		       ...
		       EndIf   		
				...
				ElseIf !lEasy .Or. (lEasy .AND. (cAliasSD1)->D1_TIPO $ "DCPI")
					...
					Else
						...
						Else
							aadd(aDi,{;
								{,,aprod[len(aprod),05]},; //1
  	   	         				{,,},; 	//2
			         	  		{,,},;	//3
        	   	   				{"I19","nDI",_nDi},;	   //4 
           	   					{"I20","dDI",_dDi},;       //5 
            	   				{"I21","xLocDesemb",_cLD},;//6 
  	            				{"I22","UFDesemb",_cUFLD},;//7   
     	         				{"I23","dDesemb",_dLD},;   //8   
        	      				{"I24","cExportador",_cForn},; //9
           	   					{"I26","nAdicao",Len(aDi)+1},; //10
              					{"I27","nSeqAdi",Len(aDi)+1},; //11
	               				{"I28","cFabricante",_cForn},; //12 
  		            			{,,},; 		//13
     		         			{0,0,0},; 	//14
        		      			{,,},; 		//15
           		   				{,,},; 		//16
        	      				...
        	      				{,,},; 		//21
	               				...
	               				{,,},; 		//24
  		            			{,,},; 		//25
								{"X06","xNome",0},;				   //26  //Claudio 16/03/2015     
								{"X07","IE",0},;				   //27
								{"X08","xEnder",0},;			   //28
								{"X09","xMun",0},;				   //29
								{"X10","UF",0},;				   //30
								...
								{"HOU","house",0},;				   //32
								{"DES","cDesp",0},;				   //33
								{"129A","nDraw",_cAcdraw},;		   //34   //Claudio	16/03/2015
								{"105a","NVE",0},;				   //35
								{"I23a","tpViaTransp",_cVtrans},;  //36   //Claudio	16/03/2015
								{"I23b","vAFRMM",""},;			   //37
								{"I23c","tpIntermedio",_cInterm},; //38   //Claudio	17/03/2015
								{"I23d","CNPJ",""},;			   //39
								{"I23e","UFTerceiro",""};          //40   //Claudio	16/03/2015
								})			                       
...
aEval(_aDescAux,{|x| IIF(x[01] == aProd[01],cString += x[02]+";","")})
cString += '</infadprod>'
...
// Ajuste Samuel Miranda 11/09/2025 - Erro 853 Sefaz
If Len(aDupl) == 1 .And. aDupl[1][2] == SF2->F2_EMISSAO
	aDupl := {}
Endif
// Fim Samuel Miranda 11/09/2025
