...
//	Ma410Impos(3,.T.,@)
	...
	If !(SC5->C5_TIPO $ "D/B")
		...
		cQuery		+= 		"C5_TPFRETE TPFRETE, "+CHR(13)+CHR(10)
		...
		cQuery		+= 		"A1_TIPOCLI TIPOCLI, "+CHR(13)+CHR(10)
	...
	Else
		...
		cQuery		+= 		"'F' TIPOCLI, "+CHR(13)+CHR(10)
	...
	Else
		...
		While !PED->(Eof())
			...
			If Len(aCabec)==0
				...
				_aCabTrib[1] := PED->CLIENTE
				_aCabTrib[2] := PED->LOJA
				_aCabTrib[3] := PED->TIPOCLI
				_aCabTrib[4] := PED->PED 
				_aCabTrib[6] := PED->EST
				_aCabTrib[7] := PED->FRETE
				_aCabTrib[8] := PED->TPFRETE
			...
			aAdd( _aItemTrib, {PED->COD, PED->TES, PED->QUANT, PED->PRCVEN, PED->VALOR} )
			_aCabTrib[5] += PED->VALOR
			...
			Transform(_oImp:GetIpiAliquota(),"@E 999.99"),;		//06 - Valor da margem do IPI
			Transform(_oImp:GetIcmsAliquota(),"@E 999.99"),;	//07 - Valor do ICM
			_oImp:GetIpiValor(),;								//08 - Valor do IPI
			_oImp:GetIcmsValor(),;								//09 - vALOR DO ICM
...
For i := 1 to len(aPrtLi)	
	...
	If aPrtLi[i,9] > 0
		//_Li	-= 30
		oPrint:Say	(_Li+15,1900,Transform(aPrtLi[i,9],"@E 999,999,999.99"),oFont8Cn )	// TOTICM
	Endif
...
//_oImp := xCalcImp( _aCabTrib, _aItemTrib)
//_nIcmGeral  := 0//_oImp:GetIcmsValor()
//_nIpiGeral	:= 0//_oImp:GetIpiValor()
//_nPisGeral	:= 0//_oImp:GetPisValor()+_oImp:GetPisApuracaoValor()
//_nCofGeral	:= 0//_oImp:GetCofinsValor()+_oImp:GetCofinsApuracaoValor()
//_nIcmsRet	:= 0//_oImp:GetIcmsSTValor()
...
oPrint:Say	(2500,0021,"Frete:",oFont10An )
oPrint:Say	(2500,0460,"Classif.Pedido:",oFont10An )
If c_MODFR == "1"
...
oPrint:Say	(2500,0950,"Transportadora:",oFont10An )     				;	oPrint:Say	(2532,0950,c_TRANSP,oFont8A )
oPrint:Say	(2500,1525,"Tel:",oFont10An )								;	oPrint:Say	(2530,1540,c_DDDT,oFont8A )
oPrint:Say	(2530,1525,"( ",oFont10An )
oPrint:Say	(2530,1555," )",oFont10An )				  					;	oPrint:Say	(2530,1620,c_FONE,oFont8A )
