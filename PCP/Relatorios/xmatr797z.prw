...
Else
	...
	//	If AllTrim(_cNumOP) <> AllTrim((_cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)) 
		...
		_nQtdAux := xNumSZ6((_cAliasTop)->C2_PRODUTO)	 // Retorna a Qtde de Item que serao impressao em outro Quadro (OPERACOES)
		...
		For I := 2 TO Len(aArray)
			...
			If !dbSeek(xFilial("SZ6")+(_cAliasTop)->C2_PRODUTO+SG2->G2_CODIGO+aArray[I][1])
				...
				oPrinter:Say(Li+010,490,aArray[I][10],oFontTB) //"LOTE"
		...
		(_cAliasTop)->(DbSkip())
		nCount--
        // Alteracao chamado 4776 - Whiverson 
		//ABmp := "O:\Desenhos em PDF\Desenhos JPG\"+AllTrim(cProduto)+".jpg" // Buscar nome em tabela especifica
        // Alteracao Bertasso tamanho do desenho
		//ABmp := "\jpg\"+AllTrim(_cProdPai)+"\"+AllTrim(_cProdPai)+"_page_1.jpg" // Buscar nome em tabela especifica
		...
		If dbSeek(xFilial("SZ2")+cProduto)
			...
			For _nCount := 1 to Len(_aBmp) // Le o Array com as Figuras a serem impressas
				...
				EndIf
	                      //////
				//_cBmp := "O:\Desenhos em PDF\Desenhos JPG\"+AllTrim(_aBmp[_nCount])+".jpg" // Nome do Arquivo Referente a Imagem
				...
				//_cDesenho   := "\dirdoc\co01\shared\" +AllTrim(_aBmp[_nCount])+".jpg

				//Se o arquivo existir, faz uma copis pra pasta Temp na maquina local.
				//If File(_cDesenho)
				//	CpyS2T(_cDesenho,"C:\temp",.F.)					
				//EndIf

				// Se encontrar o registro
				//Pega o arquivo salvo na pasta temp do , faz uma copis pra pasta Temp na maquina local.
				//_cBmp :="C:\temp\"+AllTrim(_aBmp[_nCount])+".jpg
			...
			Next
	     Else   
	   		//ABmp := "\dirdoc\co01\shared\" +AllTrim(cProduto)+".jpg
			//ABmp := "O:\Desenhos em PDF\Desenhos JPG\"+AllTrim(cProduto)+".jpg"
		...
		If lSH8
			While !Eof() .And. SH8->H8_FILIAL+SH8->H8_OP+SH8->H8_OPER == xFilial()+(_cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)+SG2->G2_OPERAC
				If !_lFirst1
					...
					oPrinter:Say(li+006,002,"OPER",oFontTB)				//"OPERACAO"
					oPrinter:Say(li+006,022,STR0047,oFontTB)				//"RECURSO"
					oPrinter:Say(li+006,172,STR0048,oFontTB)				//"FERRAMENTA"
					oPrinter:Say(li+006,322,"INSTRUÇÕES",oFontTB)		//"Instrucoes"
					oPrinter:Say(li+006,522,"Proc.Apli.DQL",oFontTB)	//"DQL"
					li+=10
				EndIf
				_lFirst1 := .T.
...
_nVezes := 0
For nBegin := 1 To Len(_cInstru) Step 40
	oPrinter:Say(li+005,322,Substr(_cInstru,nBegin,40),oFontT)
	li+=10
	_nVezes++
	If li> nMaxLin-60
		li:= 0
...
Next nBegin
Li := Li - (10*_nVezes)
oPrinter:Say(li+005,522,SH1->H1_XDQL,oFontT)
Li := Li + (10*_nVezes)
...
oPrinter:Say(Li,001,"",oFontT) // Seta a Fonte Normal (Sem Negrito)
...
_aArea := GetArea()
_cQuery := ""
_cQuery += " SELECT QP8_ENSAIO ENSAIO,QP1_DESCPO DESCRICAO,QP8_LABOR LABOR,QP8_METODO METODO,QP8_TEXTO TEXTO,' ' MINIMO,' ' MAXIMO , '1' ORDEM, QP8.R_E_C_N_O_ REG "
_cQuery += " FROM "+RetSqlName("QQK")+" QQK "
_cQuery += " INNER JOIN "+RetSqlName("QP8")+" QP8 ON "
_cQuery += " QP8_FILIAL = '"+xFilial("QP8")+"' "
_cQuery += " AND QP8_PRODUT = QQK_PRODUT "
_cQuery += " AND QP8_OPERAC = QQK_OPERAC "
...
_cQuery += " AND QP8.D_E_L_E_T_ = ' ' "
_cQuery += " INNER JOIN "+RetSqlName("QP1")+" QP1 ON QP1_FILIAL = ' ' AND QP1_ENSAIO = QP8_ENSAIO AND QP1.D_E_L_E_T_ = ' ' "
_cQuery += " WHERE QQK_FILIAL = '"+xFilial("QQK")+"' "
_cQuery += " AND QQK_PRODUT = '"+(_cAliasTop)->C2_PRODUTO+"' "
_cQuery += " AND QQK_RECURS = '"+SG2->G2_RECURSO+"' "
_cQuery += " AND QQK_OPERAC = '"+SG2->G2_OPERAC+"' "
_cQuery += " AND QQK_FERRAM = '"+SG2->G2_FERRAM+"' "
_cQuery += " AND QQK_REVIPR = QP8_REVI "
_cQuery += " AND QQK.D_E_L_E_T_ = ' ' "
_cQuery += " UNION ALL "
_cQuery += " SELECT DISTINCT QP7_ENSAIO ENSAIO,QP1_DESCPO DESCRICAO,QP7_LABOR LABOR,QP7_METODO METODO,' ' TEXTO,QP7_LIC MINIMO,QP7_LSC MAXIMO,  '2' ORDEM, QP7.R_E_C_N_O_ REG "
_cQuery += " FROM "+RetSqlName("QQK")+" QQK "
_cQuery += " INNER JOIN "+RetSqlName("QP7")+" QP7 ON "
_cQuery += " QP7_FILIAL =  '"+xFilial("QP7")+"' "
_cQuery += " AND QP7_PRODUT = QQK_PRODUT "
_cQuery += " AND QP7_OPERAC = QQK_OPERAC "
...
_cQuery += "     AND QP6_CODREC = '"+(_cAliasTop)->C2_ROTEIRO+"' "
_cQuery += "     AND QP6.D_E_L_E_T_ = ' ') "
_cQuery += " AND QP7.D_E_L_E_T_ = ' ' "
_cQuery += " INNER JOIN "+RetSqlName("QP1")+" QP1 ON QP1_FILIAL = ' ' AND QP1_ENSAIO = QP7_ENSAIO AND QP1.D_E_L_E_T_ = ' ' "
_cQuery += " WHERE QQK_FILIAL = '"+xFilial("QQK")+"' "
_cQuery += " AND QQK_PRODUT = '"+(_cAliasTop)->C2_PRODUTO+"' "
_cQuery += " AND QQK_RECURS = '"+SG2->G2_RECURSO+"' "
_cQuery += " AND QQK_OPERAC = '"+SG2->G2_OPERAC+"' "
_cQuery += " AND QQK_FERRAM = '"+SG2->G2_FERRAM+"' "
_cQuery += " AND QQK_REVIPR = QP7_REVI "
_cQuery += " AND QQK.D_E_L_E_T_ = ' ' "
...
For nI := 1 to nQtdInsp
	...
	While QRY->(!Eof())
		...
		If li > nMaxLin-60
			...
			oPrinter:Say(li+006,250,"ESPECIFICACAO"	, oFontTB)
			oPrinter:Say(li+006,552,"RESULTADO" 		, oFontTB)
			Li+=10
		EndIf
...
Static Function MarcaDagua(_cAliasTop)
