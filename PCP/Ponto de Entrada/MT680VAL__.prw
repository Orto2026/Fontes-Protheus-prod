...
// HABILIDADADO A PRDIDO DE LUIZ PCP 13/07/2023 - POR MAURICIO
...
If dbSeek(xFilial("SC2")+_cOP)
	...
	If _nSldOp < 0 // Caso o Apontamento total seja maior que o Que o saldo analisa a Tolerancia
		...
		_nSldOp := _nSldOp * -1 // Como Saldo ficou negativo, inverte para positivo
...
While !Eof() .And. SD4->D4_FILIAL == xFilial("SD4") .And. SD4->D4_OP == _cOP
	...
	If dbSeek(xFilial("SB1")+SD4->D4_COD) .And. SB1->B1_TIPO == "PI" .And. SD4->D4_LOCAL != "96"
		...
		If dbSeek(xFilial("SB8")+SD4->D4_COD+SD4->D4_LOCAL+_cLote) // Era _cLote, porem se mudar o Lote na Hora do empenho da erro pois nao havera saldo e da erro - DEMA 05/07/2017
			If (SB8->B8_SALDO + _nSldOp) < _nQuat //  ALTERADO MAURICIO 15/07/14  //SD4->D4_QTDEORI PARA _nQuat. ESTA DANDO PROBLEMA QUANTO A OP TINHA PERDA.SB8->B8_SALDO-SB8->B8_EMPENHO ALTERADO PARA SB8->B8_SALDO
...
//ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼
/*DESABILITADO POR MAURICIO - SOLICITADO POR FELIPE PCP - PROBLEMA APONTAMENTO COLETOR MES
...
//ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»//
//ºFaz a Checagem do Tipo do Apontameto Total x Parcial  //
//ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼//
                                                   
dbSelectArea("SC2")
dbSetOrder(1)
If dbSeek(xFilial("SC2")+_cOP)
	// Posiciona na OP para Buscar QTD Original e o Saldo Atual da OP
 	_nQtdOri	:= SC2->C2_QUANT
	_nSldOp 	:= ASC2SLD() 

	If _nSldOp == (_nQuat + _nPerda) .and. _cPT != "T"
	Alert("Verifique a quantidade apontada !!!")
	_lRet := .F.
	ENDIF
EndIf
