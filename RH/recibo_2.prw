...
User Function ReciboSI(lTerminal,cFilTerminal,cMatTerminal,cMesAnoRef,nRecTipo,cSemanaTerminal)
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Define Variaveis Locais (Basicas)                            ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local cString:="SRA"        // alias do arquivo principal (Base)
Local aOrd   := {"Matricula","C.Custo","Nome","Chapa","C.Custo + Nome"}
Local cDesc1 := "Emissao de Recibos de Pagamento (Singulare)."
Local cDesc2 := "Sera impresso de acordo com parametros solicitados pelo usuario"
Local cDesc3 := "Versao 1.0 | Data: 21/09/2010"
...
If !Empty(Semana) .And. Semana # "99" .And.  Upper(SRA->RA_TIPOPGT) == "S"
	...
	cSem_Ate + ")"
Else
	@ LI,82 PSAY Transform(SM0->M0_CGC , "@R 99.999.999/9999-99")+"  "+MesExtenso(MONTH(dDataRef))+"/"+STR(YEAR(dDataRef),4)
EndIf	
LI ++
@ LI,88  PSAY SRA->RA_ADMISSA	//"ADMISSAO  : "
LI ++
LI ++
LI ++
ORDEMZ ++
@ LI,00  PSAY SUBSTR(SRA->RA_MAT,1,6)		//"| Matricula : "
@ LI,08  PSAY SRA->RA_NOME	//"Nome  : "
@ LI,65  PSAY cCodFunc +" "+Substr(SRA->RA_CC,1,4)+" "+Substr(DescCc(SRA->RA_CC,SRA->RA_FILIAL),1,30)+" "+Posicione("SRJ", 1, xFilial("SRJ") + cCodFunc,"RJ_DESC")
LI ++
LI ++
LI ++
LI ++
...
For nCont := 1 To nTermina
	IF nCont <= LEN(aProve)
		...
		@ LI,64 PSAY TRANSFORM(aProve[nCont,2],'999.99')
		@ LI,85 PSAY TRANSFORM(aProve[nCont,3],cPict3)
	ENDIF
	nValidos ++
	Li ++
Next nCont
...
For nCont := 1 To nTermina
	IF nCont <= LEN(aDesco)
		@ LI,02 PSAY aDesco[nCont,1]
		@ LI,64 PSAY TRANSFORM(aDesco[nCont,2],'999.99')
		@ LI,109 PSAY TRANSFORM(aDesco[nCont,3],cPict3)
	ENDIF
	nValidos ++
	Li ++
	If nValidos = If(cPaisLoc <> "ARG",24,24)
		LI ++
		@ LI,05 PSAY "CONTINUA !!!"			// "CONTINUA !!!"
		LI ++
		LI += 8
		If li >= 85
			li := 0
    ...
    ENDIF
Next nCont
// If (ORDEMZ/2)  preciso acrescer uma linha sempre na 2nd impressao
Li := Li + (22 - nValidos)
If cPaisLoc <> "ARG"
   @ LI,005 PSAY DESC_MSG1
   @ LI,005 PSAY "CREDITO EM CONTA CORRENTE "+SRA->RA_BCDEPSAL+"-"+DescBco(SRA->RA_BCDEPSAL,SRA->RA_FILIAL)	//"| CREDITO:"   
   @ LI,084 PSAY TRANS(TOTVENC,cPict3)	//"| TOTAL BRUTO     "
   @ LI,109 PSAY TRANS(TOTDESC,cPict3)	//" TOTAL DESCONTOS     "
   LI ++
   @ LI,032 PSAY SRA->RA_CTDEPSAL		//"| CONTA:"                                                                                      
   LI ++
   LI ++ //ACRESCENTADO POR ROSSANA EM 21.09.2010 PARA ARRUMAR A IMPRESSAO SEM MONTAR NA DESCRICAO
   @ LI,109 PSAY TRANS((TOTVENC-TOTDESC),cPict3)			//"| LIQUIDO A RECEBER     "
   @ LI,005 PSAY DESC_MSG2
   LI ++
   LI ++
// LI ++ //RETIRADO POR ROSSANA EM 21.09.2010 PARA ARRUMAR A IMPRESSAO SEM MONTAR NA DESCRICAO
   LI ++ //ACRESCENTADO POR ROSSANA EM 21.09.2010 PARA ARRUMAR A IMPRESSAO SEM MONTAR NA DESCRICAO
   nTermina := LEN(aBases)
   For nCont := 1 To nTermina
   IF nCont <= LEN(aBases)                                                        
      ...
      If aBases[nCont,1] = '731'   
	     @ LI,024 PSAY TRANSFORM(aBases[nCont,3],cPict1)
         @ LI,044 PSAY TRANSFORM(aBases[nCont,3],cPict1)		
      endif                     
      If aBases[nCont,1] = '732'
         @ LI,066 PSAY TRANSFORM(aBases[nCont,3],cPict1)		      
      EndIf     
      If aBases[nCont,1] = '711'
         @ LI,089 PSAY TRANSFORM(aBases[nCont,3],cPict1)		      
      EndIf     
   ENDIF
   Next nCont
   LI ++         
