...
User Function RECIBOPG(lTerminal,cFilTerminal,cMatTerminal,cMesAnoRef,nRecTipo,cSemanaTerminal)
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Define Variaveis Locais (Basicas)                            ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local cString:="SRA"        // alias do arquivo principal (Base)
Local aOrd   := {"Matricula","C.Custo","Nome","Chapa","C.Custo + Nome"}
Local cDesc1 := "Emissao de Recibos de Pagamento Grupo Singulare."
Local cDesc2 := "Sera impresso de acordo com parametros solicitados pelo usuario"
Local cDesc3 := "Versao 2.0 | Data: 07/07/2011"
...
Private aCodFol:= {}
Private li     := _PROW()
Private Titulo := "EMISSAO DE RECIBOS DE PAGAMENTOS - GRUPO SINGULARE V2"
Private lEnvioOk := .F.
Private c_FilAnt := cFilAnt

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Envia controle para a funcao SETPRINT                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
wnrel:="RECIBOPG"            //Nome Default do relatorio em Disco
...
±±³Rossana(Focus)³07/07/11³Tratado para imprimir os dados da empresa atual³±±  //chamado: 6492
±±³              ³  /  /  ³                                               ³±±
±±³              ³  /  /  ³                                               ³±±
...
If !Empty(Semana) .And. Semana # '99' .And.  Upper(SRA->RA_TIPOPGT) == 'S'
	...
	cSem_Ate + ')'
Else
	@ LI,35 PSAY SM0->M0_CGC+"   "+MesExtenso(MONTH(dDataRef))+"/"+STR(YEAR(dDataRef),4)
EndIf
...
//	SM0->(DbSeek(SM0->M0_CODIGO+SRA->RA_FILIAL))
	SM0->(DbSeek(c_CodEmp+SRA->RA_FILIAL))
	
	cFilAnt := SM0->M0_CODFIL
	
	LI ++
	@ LI,60 PSAY "EMPRESA : "+ SM0->M0_NOMECOM
	LI ++
	@ LI,60 PSAY "C.N.P.J.: "+ Transform(SM0->M0_CGC , "@R 99.999.999/9999-99")+"  -  Ref.: "+MesExtenso(MONTH(dDataRef))+"/"+STR(YEAR(dDataRef),4)
...
LI ++
ORDEMZ ++
@ LI,01		PSAY SUBSTR(SRA->RA_MAT,1,6)		//"| Matricula : "
@ LI,08		PSAY SRA->RA_NOME	//"Nome  : "
@ LI,63		PSAY cCodFunc
@ LI,72		PSAY SRA->RA_CC
@ LI,85 	PSAY SRA->RA_ADMISSA
@ LI,97		PSAY Posicione("SRJ", 1, xFilial("SRJ") + cCodFunc,"RJ_DESC")
LI ++
LI ++
LI ++
...
For nCont := 1 To nTermina
	IF nCont <= LEN(aProve)
		@ LI,00		PSAY aProve[nCont,1]
		@ LI,61		PSAY TRANSFORM(aProve[nCont,2],'999.99')
		@ LI,80		PSAY TRANSFORM(aProve[nCont,3],cPict3)
	ENDIF
	nValidos ++
	Li ++
	If cPaisLoc <> "ARG" .and. nValidos > 16
		LI ++
		@ LI,05 PSAY "CONTINUA IMPRESSAO DE PROVENTOS !!!"			// "CONTINUA !!!"
		LI += 8
/*		If li >= 85
			li := 0
		Endif      */
...
For nCont := 1 To nTermina
	IF nCont <= LEN(aDesco)
		@ LI,00		PSAY aDesco[nCont,1]
		@ LI,61		PSAY TRANSFORM(aDesco[nCont,2],'999.99')
		@ LI,103	PSAY TRANSFORM(aDesco[nCont,3],cPict3)
	ENDIF
	nValidos ++
	Li ++
	//	If nValidos = If(cPaisLoc <> "ARG",24,24)
	If cPaisLoc <> "ARG" .and. nValidos > 16
		LI ++
		@ LI,05 PSAY "CONTINUA IMPRESSAO DE DESCONTOS !!!"			// "CONTINUA !!!"
		LI += 8
/*		If li >= 85
			li := 0
		Endif*/
...
If cPaisLoc <> "ARG"
	...
	@ LI,000 PSAY "CREDITO EM CONTA CORRENTE "+SRA->RA_BCDEPSAL+"-"+DescBco(SRA->RA_BCDEPSAL,SRA->RA_FILIAL)	//"| CREDITO:"
	@ LI,080 PSAY TRANS(TOTVENC,cPict3)	//"| TOTAL BRUTO     "
	@ LI,103 PSAY TRANS(TOTDESC,cPict3)	//" TOTAL DESCONTOS     "
	LI ++
	@ LI,032 PSAY SRA->RA_CTDEPSAL		//"| CONTA:"
	LI ++
	@ LI,103 PSAY TRANS((TOTVENC-TOTDESC),cPict3)			//"| LIQUIDO A RECEBER     "
	...
	For nCont := 1 To nTermina
		IF nCont <= LEN(aBases)
			If aBases[nCont,1] = '834'
				@ LI,005 PSAY TRANSFORM(aBases[nCont,3],cPict1)
			EndIf
			If aBases[nCont,1] = '731'
				@ LI,023 PSAY TRANSFORM(aBases[nCont,3],cPict1)
				@ LI,046 PSAY TRANSFORM(aBases[nCont,3],cPict1)
			endif
			If aBases[nCont,1] = '732'
				@ LI,064 PSAY TRANSFORM(aBases[nCont,3],cPict1)
			EndIf
			If aBases[nCont,1] = '711'
				@ LI,088 PSAY TRANSFORM(aBases[nCont,3],cPict1)
			EndIf
		ENDIF
	Next nCont
	If cPaisLoc <> "ARG" .and. nValidos <= 16
...
/*If LI > 66
	LI := 0
//	SetPrc(0,0) //Serve para zerar a impressão e começar novamente...
//	Li := 4
EndIf*/  
...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ Generico                                                   ³±±
5±±ÃÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function fContinua()    // Continuacao do Recibo

Li+=1
@ LI,05 PSAY &cNormal + "CONTINUA !!!"		//"CONTINUA !!!"
Li+=8
...
Local cHtml			:= ""
Local cSubject		:= "DEMONSTRATIVO DE PAGAMENTO "	//" DEMONSTRATIVO DE PAGAMENTO "
...
IF Esc == 1
	...
	cTipo		:= "Adiantamento" // "Adiantamento"
...
ElseIF Esc == 3
	aGetArea	:= SRC->( GetArea() )
	cTipo := "1a. parcela 13o. " //"1a. Parcela do 13o."
...
ElseIF Esc == 4
	...
	cTipo := "2a parcela 13o." //"2a. Parcela do 13o."
	...
	EndIF
ElseIF Esc == 5
	cTipo		:= "Valores Extras" //"Valores Extras"
	cVerbaLiq	:= ""
EndIF
...
IF ( !Empty(Semana) .and. ( Semana # "99" ) .and. ( Upper(SRA->RA_TIPOPGT) == "S" ) )
	...
	ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ*/
	PerSemana()
	cReferencia := "Semana" + Semana + " (" + cSem_De + " a " +	cSem_Ate + ")" //"Semana  "###" a "
Else
...
IF !( lTerminal )
	...
	cHtml += 					'<h4 size="03">'
	cHtml +=					'<br>'
	cHtml += 						"DEMONSTRATIVO DE PAGAMENTO" // " DEMONSTRATIVO DE PAGAMENTO "
	cHtml += 					'<br>'
...
IF !( lTerminal )
	...
	cHtml += 	'&nbsp;&nbsp;&nbsp' + "Funcao" + cCodFunc + "  "+cDescFunc	+'<br>' //"Funcao    - "
	cHtml +=  	'&nbsp;&nbsp;&nbsp' + "C. Custos" + SRA->RA_CC + " - " + DescCc(SRA->RA_CC,SRA->RA_FILIAL) +'<br>' //"C.Custo   - "
...
Else
	...
	cHtml += 									'<span class="etiquetas"><div align="Left">'+ "Codigo" + '</span></div>' + CRLF //C&oacute;digo
	cHtml += 								'</td>' + CRLF
	cHtml += 								'<td width="219" valign="top">' + CRLF
	cHtml += 									'<span class="etiquetas"><div align="left">' + "Descricao" + '</span></div>' + CRLF //Descri&ccedil;&atilde;o
	cHtml += 								'</td>' + CRLF
	cHtml += 								'<td width="127" valign="top">' + CRLF
	cHtml += 									'<span class="etiquetas"><div align="right">' + "Referencia"  + '</span></div>' + CRLF //Refer&ecirc;ncia
	cHtml += 								'</td>' + CRLF
	cHtml += 								'<td width="127" valign="top">' + CRLF
	cHtml += 									'<span class="etiquetas"><div align="right">' + "Valores" + '</span></div>' + CRLF //Valores
...
IF !( lTerminal )
	...
	cHtml +=	'</tr>'
	cHtml += 	'<tr>'
	cHtml += 		'<td class="tdPrinc">' + "Total de Descontos" + '</td>' //"Total Descontos "
	cHtml += 		'<td class="td18_94_AlignR"></Td>'
...
Else
	...
	cHtml += 							'<tr>' + CRLF
	cHtml += 								'<td width="219" valign="top" bgcolor="#FAFBFC">' + CRLF
	cHtml += 									'<div align="left" class="etiquetas">' + "Liquido a Receber"  + '</div>' + CRLF //"L&iacute;quido a Receber: "
	cHtml += 								'</td>' + CRLF
	cHtml += 								'<td width="45" valign="top" bgcolor="#FAFBFC">' + CRLF
	cHtml += 									'<div align="left" class="etiquetas"> </div>' + CRLF
	cHtml += 								'</td>' + CRLF
...
ElseIf Esc = 2 .Or. Esc = 4
	...
	IF cBaseAux = "S"
		...
		Else
			...
			cHtml +=										"Base FGTS/Valor FGTS" + CRLF //"Base FGTS/Valor FGTS"
			cHtml +=									"</td>" + CRLF
			...
			cHtml +=										"Base IRRF Folha/Ferias" + CRLF //"Base IRRF Folha/Ferias"
			cHtml += 									"</td>" + CRLF
...
cHtml +=					"<b>" + CRLF
cHtml += 						"<span class='titulo_opcao'>" + Capital( "DEMONSTRATIVO DE PAGAMENTO" ) + "</span>" + CRLF //DEMONSTRATIVO DE PAGAMENTO
cHtml +=					"</b>" + CRLF
cHtml +=		"</TBODY>" + CRLF
cHtml +=	"</TABLE>" + CRLF
...
cHtml +=					"<P align=left>" + CRLF
cHtml +=						"<span class='etiquetas'>" + "Endereco:" + CRLF //"Endereço:"
cHtml +=						"</span>" + CRLF
...
cHtml +=					"<P align=left>" + CRLF
cHtml +=						"<span class='etiquetas'>" + "Credito em " + CRLF //"Crédito em:"
cHtml +=						"</span>" + CRLF
cHtml +=						"<span class='dados'>" + CRLF
...
cHtml +=					"<P align=left>" + CRLF
cHtml +=						"<span class='etiquetas'>" + "Banco/Agencia/Conta" + CRLF //"Banco/Agência/Conta:"
cHtml +=						"</span>" + CRLF
cHtml +=						"<span class='dados'>" + CRLF
...
cHtml +=				"<TD vAlign=top width='100%' height=1>" + CRLF
cHtml +=					"<P align=left>" + CRLF
cHtml +=						"<span class='etiquetas'>" +  "Referencia" + CRLF //"Referência:"
cHtml += 						"</span>" + CRLF
cHtml +=						"<span class='dados'>" + CRLF
...
cHtml +=				"<TD vAlign=top width='40%' height=1>" + CRLF
cHtml +=					"<P align=left>" + CRLF
cHtml +=						"<span class='etiquetas'>" + "Salario Nominal" + CRLF //Salário Nominal:
cHtml += 						"</span>" + CRLF
cHtml +=						"<span class='dados'>" + CRLF
...
cHtml +=					"<P align=left>" + CRLF
cHtml +=						"<span class='etiquetas'>" + "Admissao:" + CRLF //"Admissão:"
cHtml += 						"</span>" + CRLF
cHtml +=						"<span class='dados'>" + CRLF
...
cHtml +=					"<P align=left>" + CRLF
cHtml +=						"<span class='etiquetas'>" + "Dependente(s) Salario Familia" + CRLF //"Dependente(s) Salário Família:"
cHtml += 						"</span>" + CRLF
cHtml +=						"<span class='dados'>" + CRLF
